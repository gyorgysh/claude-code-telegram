...
- **Voice** (`src/telegram/voice.ts`): voice notes are downloaded and POSTed to an OpenAI-compatible `/audio/transcriptions` endpoint; the transcript is echoed and then run as a normal text turn. Disabled (with a hint) when `OPENAI_API_KEY` is unset.
